...
class PPO:
    ...
    def __init__(self, policy_class, env, load_model=False, **hyperparameters):
        ...
        self.env = env
        print(
            f"Rotation threshold : {self.env.rotation_threshold} rad.\nDistance threshold : {self.env.distance_threshold}m."
        )
    ...
    def plot_rewards(self):
        rewards = np.array(total_rewards).ravel().ravel()

        rewards = [ep_rews for ep_rews in rewards]
        # print(rewards)

        # exit()
        plt.plot(rewards)
        plt.title("Rewards vs Timestep Graph")
        plt.xlabel("Timestep")
        plt.ylabel("Rewards")
        plt.show()

    def plot_losses(self):
        losses = np.array(total_losses).ravel()
        plt.plot(losses)
        plt.title("Losses vs Timestep Graph")
        plt.xlabel("Timestep")
        plt.ylabel("Losses")
        plt.show()
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:  # ALG STEP 2
            ...
            for _ in range(self.n_updates_per_iteration):  # ALG STEP 6 & 7
                ...
                # print(ratios)
                # Calculate clip surrogate losses.
                surr1 = ratios * A_k
    ...
    def rollout(self):
        ...
        while t < self.timesteps_per_batch:
            ...
            for ep_t in range(self.max_timesteps_per_episode):
                ...
                self.prev_dist = dist
                obs, dist, done, _ = self.env.step(action.reshape((20)))
                # print(obs)
                rew = (
                    self.prev_dist - dist - 1
                )  # Implements the r = d(t) - d(t+1) condition for reward
                # print(rew)

                # curr_dist = calc_distance(obs)
                # rew = prev_dist-curr_dist
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        self.save_freq = 1000  # How often we save in number of iterations
        self.seed = (
    ...
    def _log_summary(self):
        ...
        total_rewards.append(self.logger["batch_rews"])
        ...
        total_losses.append([losses.float().mean() for losses in self.logger["actor_losses"]])
